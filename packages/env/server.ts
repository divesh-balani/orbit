...
function createServerEnv() {
	return createEnv({
		server: {
			...
			WEB_URL: z
				.string()
				.describe("Public URL of the server eg. https://orbit.so"),
			...
			// In this case, ORBIT_AWS_BUCKET_URL should be your CloudFront distribution's URL
			ORBIT_AWS_BUCKET_URL: z
				.string()
				.optional()
				.describe("Public URL of the S3 bucket"),
			ORBIT_CLOUDFRONT_DISTRIBUTION_ID: z.string().optional(),
			...
			/// Settings
			ORBIT_VIDEOS_DEFAULT_PUBLIC: boolString(true).describe(
				"Should videos be public or private by default",
			),
			ORBIT_ALLOWED_SIGNUP_DOMAINS: z
				.string()
				.optional()
			...
			/// Orbit Cloud
			// These are only needed for Orbit Cloud (https://orbit.so)
		...
		experimental__runtimeEnv: {
			S3_PUBLIC_ENDPOINT: process.env.ORBIT_AWS_ENDPOINT,
			S3_INTERNAL_ENDPOINT: process.env.ORBIT_AWS_ENDPOINT,
			...process.env,
