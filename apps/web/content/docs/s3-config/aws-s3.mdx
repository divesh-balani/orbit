---
title: "Orbit Apps: S3 Config (AWS)"
summary: "Connect your own S3 bucket to Orbit and maintain 100% ownership over your data."
tags: "Orbit Apps"
image: "/docs/s3-config/aws-s3.jpg"
---

# Connecting Your Amazon S3 Bucket to Orbit Desktop

> **TL;DR**: Open Orbit Desktop → Orbit Apps → S3 Config → Configure → Select Amazon S3 → Enter your AWS credentials → Configure bucket permissions → Save and test with a shareable link upload.
>
> **Note**: While this guide focuses on AWS S3, Orbit supports any S3-compatible storage service (like Supabase, MinIO, DigitalOcean Spaces, or Backblaze B2). The setup process is similar - you'll just need the appropriate endpoint URL and credentials from your provider.

## Step 1: Launch Orbit Desktop and Access Orbit Apps

1. **Open the Orbit Desktop App**: Click the Orbit icon on your desktop or find it in your applications folder to launch the app.

2. **Navigate to Orbit Apps**: Once the app is open, locate the 'Orbit Apps' tab in Settings.
...
1. **Create S3 Bucket First**:
   ...
   - Navigate to S3
   - Click "Create bucket"
   - Enter a unique bucket name (e.g., "my-orbit-uploads")
...
3. **Create IAM User**:
   ...
   - Set username as "orbit-app-user"
   - Click "Next"
   - Choose "Attach policies directly"
   - Search for and select the "CapAppS3Access" policy you just created
   - Click through to review
   - Create the user
   - After creation, click on the username "orbit-app-user" to open user details
     ...
     "Statement": [
       {
         ...
         "Effect": "Allow",
         "Principal": {
           "AWS": "arn:aws:iam::YOUR_ACCOUNT_ID:user/orbit-app-user"
         },
...
## Step 4: Configure Orbit Desktop

1. Open Orbit Desktop and navigate to S3 Config
...
1. Test the Connection: Click on the 'Test Connection' button to verify that Orbit can successfully connect to your S3 bucket.
2. Save the Configuration: If the test is successful, save the connection settings. Your Amazon S3 bucket is now integrated with Orbit Desktop.

## Accessing Your S3 Files in Orbit
