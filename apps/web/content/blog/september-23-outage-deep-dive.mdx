...
- During `sst deploy` the Vercel project that serves orbit.so was removed, which took down marketing pages, the dashboard, video sharing pages, and APIs used by Orbit Desktop.
...
- **Surface area:** orbit.so marketing site, dashboard, video sharing, and desktop upload APIs.
- **Customer effect:** broken links and failed API calls from Orbit Desktop during the incident window.
...
- **Shortly after**: `sst deploy` removed the Vercel project that hosts orbit.so, taking down web and API surfaces.
...
- **4:30 pm**: Found a redirect from `orbit.so` to `www.orbit.so` that swallowed `Authorization` headers. Fixed the routing and header handling. 100 percent of orbit.so back online.
...
/// in app()
// what we had
new vercel.Project("VercelProject", { name: "orbit-web" });

// what we expected to use across stages
vercel.getProjectOutput({ name: "orbit-web" });
```
...
After most pages returned, uploads from Orbit Desktop still failed. We had configured orbit.so to www.orbit.so as recommended by Vercel, but this had a side-effect of causing the Authorization header to be stripped from any requests to orbit.so. The client sent the header, the redirect hop discarded it, and the target saw an unauthenticated request. We removed this redirect and video uploads began working again.
...
- Orbit's local Studio Mode remained fully functional. Users could still record videos locally, export them, or save them for later to create shareable links once service was restored.
