...
export const SharingDialog: React.FC<SharingDialogProps> = ({
	...
	const updateSharing = useMutation({
		mutationFn: async ({
			...
			const result = await shareOrbit({ orbitId, spaceIds, public: isPublic });
			...
			if (passwordEnabled && passwordValue.trim()) {
				const pwResult = await setVideoPassword(orbitId, passwordValue);
				if (!pwResult.success) {
					throw new Error(pwResult.error || "Failed to set password");
				}
			} else if (!passwordEnabled && initialPasswordEnabled) {
				const pwResult = await removeVideoPassword(orbitId);
				if (!pwResult.success) {
	...
	const embedCode = useEmbedCode(orbitId);
		...
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent className="p-0 w-full max-w-md rounded-xl border bg-gray-2 border-gray-4">
				<DialogHeader
					...
					description={
						activeTab === "Share"
							? "Select how you would like to share the orbit"
							: "Copy the embed code to share your orbit"
					}
				>
...
function useEmbedCode(orbitId: Video.VideoId) {
	...
	<div style="position: relative; padding-bottom: 56.25%; height: 0;">
			<iframe
			src="${publicEnv.webUrl}/embed/${orbitId}"
			frameborder="0"
