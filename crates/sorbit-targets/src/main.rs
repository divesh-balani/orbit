...
fn main() {
    ...
    for (index, display) in sorbit_targets::Display::list().iter().enumerate() {
        ...
        let main_display_id = sorbit_targets::Display::list().first().map(|d| d.id());
    ...
    if let Some(cursor_display) = sorbit_targets::Display::get_containing_cursor() {
        println!(
    ...
    let windows = sorbit_targets::Window::list_containing_cursor();
    ...
    if let Some(topmost) = sorbit_targets::Window::get_topmost_at_cursor()
    ...
    loop {
        let mut relevant_windows = sorbit_targets::WindowImpl::list_containing_cursor()
            .into_iter()
            .filter_map(|window| {
