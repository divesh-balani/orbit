...
mod test {
    ...
    #[test]
    fn auto_segment_uses_focus_position() {
        ...
        let focus = Coord::new(XY::new(0.8, 0.25));
        let zoom = InterpolatedZoom::new_with_cursor(c(2.8, &segments), focus, Some(focus));

        assert!(zoom.bounds.top_left.x < -0.2);
        assert!(zoom.bounds.top_left.y > -0.2);
    }
}
